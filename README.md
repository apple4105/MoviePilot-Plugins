...
| 2  |  [自定义音频 Provider](docs/CustomAudio.md)  | v1.4 | 基于 OpenAI 兼容接口的 ASR/TTS 语音插件，支持语音识别与语音合成，启用后自动切换系统音频 provider，支持 Telegram 语音收发与动态配置。 | 需要认证 |
| 3  |  [剧集订阅增强](docs/SubscribeStatusFiller.md)  | v1.0.0 | 根据剧集播出状态自动调整订阅策略：已完结剧下完整剧集包，连载剧只追已播集数。 | 需要认证 |
