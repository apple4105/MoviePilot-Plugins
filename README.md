...
| 2  |  [自定义音频 Provider](docs/CustomAudio.md)  | v1.4 | 基于 StepFun API 的 ASR/TTS 语音插件，支持 OpenAI Whisper 兼容语音识别与 TTS 语音合成，启用后自动切换系统音频 provider，支持 Telegram 语音收发与动态配置。 | 需要认证 |

### 自定义音频 Provider 更新日志

#### v1.4 (2026-08-04)
- ASR/TTS 测试按钮与区块标题按测试状态动态变色（成功绿色 / 失败红色 / 未测试默认色）
- 配置项（API 地址、Key、模型、音色）变更后自动重置测试状态
