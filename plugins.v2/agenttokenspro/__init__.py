...
class AgentTokensPro(_PluginBase):
    ...
    def record_tokens_usage(self, event: Event):
        ...
        with self._usage_lock:
            ...
            # 所有业务请求都累加调用次数（成功+失败），不再仅依赖事件中的 model_call_count
            event_call_count = max(self._to_int(self._event_get(event.event_data, "model_call_count"), 0), 0)
            record["model_call_count"] = self._to_int(record.get("model_call_count"), 0) + max(event_call_count, 1)
