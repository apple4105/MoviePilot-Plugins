...
import { _ as _export_sfc, f as formatTokens, u as unwrapResponse } from './_plugin-vue_export-helper-BYTvrSm4.js';
...
async function loadStatus() {
  ...
  error.value = '';
  try {
    const response = await props.api.get('plugin/AgentTokensPro/status');
                      ...
                      (visibleProviders.value.length)
                        ? (_openBlock(), _createElementBlock("div", _hoisted_11, [
                            (_openBlock(true), _createElementBlock(_Fragment, null, _renderList(visibleProviders.value, (row) => {
                              return (_openBlock(), _createElementBlock("div", {
                                key: row.id,
                                class: _normalizeClass(["agenttokens-dashboard-provider", { 'agenttokens-dashboard-provider--active': row.id === activeProviderId.value }])
                              }, [
                                _createVNode(_component_VIcon, {
                                  icon: row.id === activeProviderId.value ? 'mdi-radiobox-marked' : (row.usage?.exhausted ? 'mdi-alert-circle' : 'mdi-radiobox-blank'),
                                  color: row.id === activeProviderId.value ? 'primary' : (row.usage?.exhausted ? 'error' : undefined),
                                  size: "16"
                                }, null, 8, ["icon", "color"]),
                                _createElementVNode("div", _hoisted_12, [
                                  _createElementVNode("div", {
                                    class: _normalizeClass(["agenttokens-dashboard-provider__name", { 'agenttokens-dashboard-provider__name--faulty': isProviderFaulty(row) }])
...
const Dashboard = /*#__PURE__*/_export_sfc(_sfc_main, [['__scopeId',"data-v-fce87a90"]]);
