...
### v1.5 (2026-08-06)
- **重构**：表单布局重写为 JS 注入样式的 `ca-form-item` 方案，解决 Vite CSS 过滤导致样式丢失问题
- **优化**：桌面端 `merged` 行（API 地址 + 模型）并排对齐，标签 `line-height: 40px` 精确居中
- **优化**：移动端 `merged` 行纵向堆叠，API 地址单独一行，适配小屏
- **优化**：所有控件统一 `density="compact"`，控件高度 40px 对齐
